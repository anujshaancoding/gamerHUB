...
export function CommunityPostPage() {
  ...
  useEffect(() => {
    ...
    if (showPostMenu) document.addEventListener("mousedown", handleClickOutside);
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      router.push("/community?tab=blog");
    } catch (err) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        ...
        <div className="flex items-center gap-3 pb-6 mb-8 border-b border-border">
          ...
          {/* Author: Edit & More menu */}
          {isAuthor && (
            <>
              <Button
                variant="outline"
                size="sm"
                onClick={() => router.push(`/write?edit=${post.slug}`)}
                ...
                {showPostMenu && (
                  <div className="absolute right-0 top-full mt-1 w-40 bg-surface border border-border rounded-lg shadow-xl z-20 overflow-hidden">
                    <button
                      onClick={() => {
                        setShowPostMenu(false);
                        router.push(`/write?edit=${post.slug}`);
                      }}
                      className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-text hover:bg-surface-light transition-colors"
                    >
      ...
      <ConfirmDeleteDialog
        ...
        onOpenChange={(open) => !open && setShowPostDeleteConfirm(false)}
        onConfirm={handleDeletePost}
        title="Delete this post?"
        description="This post and all its comments will be permanently removed. This action cannot be undone."
      />
    </div>
  );
}
