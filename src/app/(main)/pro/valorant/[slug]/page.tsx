...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const description = player.bio
    ...
    : `${player.ign} — Indian Valorant pro${player.team ? ` for ${player.team.name}` : ""}. Stats, gear, sensitivity, and crosshair code.`;
  return {
    title,
    description,
    openGraph: {
...
export default async function ValorantProPlayerPage({ params }: Props) {
  const { slug } = await params;
  const detail = await getProPlayerDetail("valorant", slug);
  if (!detail) notFound();
