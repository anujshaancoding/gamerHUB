...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const detail = await getProPlayerDetail("freefire", slug);
  if (!detail) return { title: "Player Not Found · ggLobby" };

  const { player } = detail;
  const title = `${player.ign}${player.real_name ? ` (${player.real_name})` : ""} — Free Fire Pro · ggLobby`;
  const description = player.bio
    ? player.bio.slice(0, 160)
    : `${player.ign} — Indian Free Fire pro${player.team ? ` for ${player.team.name}` : ""}. Stats, characters, sensitivity, device.`;
  return {
    title,
    description,
    openGraph: {
...
export default async function FreefireProPlayerPage({ params }: Props) {
  const { slug } = await params;
  const detail = await getProPlayerDetail("freefire", slug);
  if (!detail) notFound();
